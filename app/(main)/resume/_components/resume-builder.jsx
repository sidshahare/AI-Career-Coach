...
export default function ResumeBuilder({ initialContent }) {
  ...
  const entriesToMarkdown = (entries, sectionTitle) => {
    ...
    const entriesContent = entries
      .map((entry) => {
        const entryParts = [];
        if (entry.title) entryParts.push(`**${entry.title}**`);
        if (entry.company) entryParts.push(`*${entry.company}*`);
        if (entry.dates) entryParts.push(`*${entry.dates}*`);
        if (entry.description)
          entryParts.push(entry.description.replace(/\n/g, "  \n"));
